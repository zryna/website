---
title: "M2 straight-line semantics"
description: "Compiler-owned 0.1.0 documentation imported from f4d28002a014."
slug: "reference/compiler/0.1.0/reference/m2-straight-line-semantics"
---

> Verified compiler source: [docs/M2_STRAIGHT_LINE_SEMANTICS.md](https://github.com/zryna/zryna/blob/f4d28002a014cd2e717eba4e59764bd925bef8c1/docs/M2_STRAIGHT_LINE_SEMANTICS.md) at commit `f4d28002a014cd2e717eba4e59764bd925bef8c1`.
...
[M2 control-flow semantic boundary](/reference/compiler/0.1.0/reference/m2-control-flow-semantics/), which owns canonical `if` and
`while` lowering and definite state. The sealed internal
[M2 JavaScript backend](/reference/compiler/0.1.0/reference/m2-javascript-backend/) now accepts the verified program, and the
[M2 core WebAssembly backend](/reference/compiler/0.1.0/reference/m2-webassembly-backend/) accepts the same authority. The
[M2 native MIR profile](/reference/compiler/0.1.0/reference/m2-native-mir/) independently reseals it into target-specific verified
views, and the internal [M2 Linux x86-64 native backend](/reference/compiler/0.1.0/reference/m2-native-backend/) emits, links, and
executes the audited object. The explicit public profile and manifest v2 are available through the
atomic M2 driver path. The independent [three-target conformance gate](/reference/compiler/0.1.0/reference/m2-conformance/) is
...
[manifest-v2 atomic transaction](/reference/compiler/0.1.0/reference/m2-manifest-v2/). The separate
[fixed-oracle aggregate M2 gate](/reference/compiler/0.1.0/reference/m2-conformance/) proves equivalence without turning it into a
