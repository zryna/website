...
> Verified compiler source: [docs/M2_STRAIGHT_LINE_SEMANTICS.md](https://github.com/zryna/zryna/blob/0b80816b7bca4d619c4716f1f15c993b30edb613/docs/M2_STRAIGHT_LINE_SEMANTICS.md) at commit `0b80816b7bca4d619c4716f1f15c993b30edb613`.
...
[M2 control-flow semantic boundary](/reference/compiler/next/reference/m2-control-flow-semantics/), which owns canonical `if` and
`while` lowering and definite state. The sealed internal
[M2 JavaScript backend](/reference/compiler/next/reference/m2-javascript-backend/) now accepts the verified program, and the
[M2 core WebAssembly backend](/reference/compiler/next/reference/m2-webassembly-backend/) accepts the same authority. The
[M2 native MIR profile](/reference/compiler/next/reference/m2-native-mir/) independently reseals it into target-specific verified
views, and the internal [M2 Linux x86-64 native backend](/reference/compiler/next/reference/m2-native-backend/) emits, links, and
executes the audited object. The explicit public profile and manifest v2 are available through the
atomic M2 driver path. The independent [three-target conformance gate](/reference/compiler/next/reference/m2-conformance/) is
...
[manifest-v2 atomic transaction](/reference/compiler/next/reference/m2-manifest-v2/). The separate
[fixed-oracle aggregate M2 gate](/reference/compiler/next/reference/m2-conformance/) proves equivalence without turning it into a
