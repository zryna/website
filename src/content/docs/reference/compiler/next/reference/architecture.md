...
> Verified compiler source: [docs/ARCHITECTURE.md](https://github.com/zryna/zryna/blob/4c9fbda9ca80decf755fb8313474217e051eb5c8/docs/ARCHITECTURE.md) at commit `4c9fbda9ca80decf755fb8313474217e051eb5c8`.

# Compiler architecture

Current public M3 selection is `--profile data-ownership-v1`, using protocol v4 and manifest v3.
See [the public M3 surface](/reference/compiler/next/reference/m3-public-profile/) and [the beginner guide](/reference/compiler/next/reference/m3-getting-started/).
...
[M1 conformance suite](/reference/compiler/next/status/m1-conformance/) compares the public M1 `all` observations with
fixed expected values and the committed manifest; the runtime command does not define a second
comparison semantics. The [M2 conformance gate](/reference/compiler/next/reference/m2-conformance/) independently performs the
fixed-oracle aggregate M2 comparison without adding a runtime semantics authority.
See the [CLI reference](/reference/compiler/next/reference/cli/) and [manifest-v2 contract](/reference/compiler/next/reference/m2-manifest-v2/) for the exact command, layout, manifest,
...
[candidate driver contract](/reference/compiler/next/reference/m3-candidate-driver/).
...
[scalar control-flow and modules v1](/reference/compiler/next/reference/control-flow-modules-v1/) specification.
...
[bounded deterministic module closure](/reference/compiler/next/reference/m2-module-closure/) that safely resolves an explicit
...
[straight-line M2 semantic boundary](/reference/compiler/next/reference/m2-straight-line-semantics/) revalidates the final graph and
lowers exact types, locals, lexical scopes, arithmetic, comparisons, assignment, and acyclic direct
calls. The internal [control-flow semantic boundary](/reference/compiler/next/reference/m2-control-flow-semantics/) extends that
authority with canonical branches, loops, merge/header parameters, definite state, reachability,
and return analysis. The internal [M2 JavaScript backend](/reference/compiler/next/reference/m2-javascript-backend/) and
[direct core WebAssembly backend](/reference/compiler/next/reference/m2-webassembly-backend/) consume only the resulting opaque
...
entrypoints. The separately versioned [M2 native MIR profile](/reference/compiler/next/reference/m2-native-mir/) lowers the same
...
[M2 native object and typed link/run boundary](/reference/compiler/next/reference/m2-native-backend/) emits local typed bodies and
...
and one [manifest-v2 transaction](/reference/compiler/next/reference/m2-manifest-v2/). Individual M2 target requests and the
independent [fixed-oracle three-target conformance gate](/reference/compiler/next/reference/m2-conformance/) are implemented. Issue
...
The isolated [`DataOwnershipV1` IR contract](/reference/compiler/next/reference/m3-data-ownership-ir/) verifies one raw program
...
The separate [`ownership-runtime ABI authority`](/reference/compiler/next/reference/m3-ownership-runtime-abi/) now verifies the exact
...
The checked [non-indexed owned borrowing matrix](https://github.com/zryna/zryna/blob/4c9fbda9ca80decf755fb8313474217e051eb5c8/docs/M3_NONINDEXED_OWNED_BORROWING_MATRIX.md)
...
authority is reconciled by the [#269 source-completion proof](https://github.com/zryna/zryna/blob/4c9fbda9ca80decf755fb8313474217e051eb5c8/docs/M3_SOURCE_COMPLETION.md).

The checked [internal owned-call closure matrix](https://github.com/zryna/zryna/blob/4c9fbda9ca80decf755fb8313474217e051eb5c8/docs/M3_OWNED_CALL_CLOSURE_MATRIX.md) integrates #329
...
The checked [complete enum matching closure matrix](https://github.com/zryna/zryna/blob/4c9fbda9ca80decf755fb8313474217e051eb5c8/docs/M3_COMPLETE_ENUM_MATCHING_MATRIX.md) integrates
...
The checked [structured owned control-flow matrix](https://github.com/zryna/zryna/blob/4c9fbda9ca80decf755fb8313474217e051eb5c8/docs/M3_STRUCTURED_OWNED_CONTROL_FLOW_MATRIX.md)
...
The [internal M3 target backends](https://github.com/zryna/zryna/blob/4c9fbda9ca80decf755fb8313474217e051eb5c8/docs/M3_TARGET_BACKENDS.md) consume the resulting closed operand and
...
The internal [`M3 Copy aggregate semantic boundary`](/reference/compiler/next/reference/m3-copy-aggregate-semantics/) consumes the
...
[`DATA_OWNERSHIP_V1.md`](/reference/compiler/next/reference/data-ownership-v1/),
[`AGGREGATE_LAYOUT_V1.md`](/reference/compiler/next/reference/aggregate-layout-v1/), and
[`OWNERSHIP_RUNTIME_V1.md`](/reference/compiler/next/reference/ownership-runtime-v1/). The digest-pinned
...
The [bounded borrowing implementation contract](/reference/compiler/next/reference/m3-borrowing-semantics/) freezes Issue #82 as
...
[`SYNTAX_PROTOCOL_V4.md`](/reference/compiler/next/reference/syntax-protocol-v4/).
